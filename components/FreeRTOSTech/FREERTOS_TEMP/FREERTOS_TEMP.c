...
void vDELETE_TEMP(void *pvParameters){
    while (1) {
    }
}
...
//开始任务任务函数
void START_TASK_TEMP(void *pvParameters)
{
    ...
    xTaskCreate(
            ...
            (TaskHandle_t *)                         &TASK2_HANDLER_TEMP // 任务句柄，任务创建成功以后会返回次惹怒我的任务句柄， 这个 句柄其实就是任务的 任务堆栈，此参数 就用来保存这个任务句柄；其他API函数可能会使用到这个 句柄
    );

    vTaskDelete(START_TASK_HANDLER_TEMP); //删除开始任务;  为什么执行完成要删除？？？
...
void FREERTOS_TEMP_INIT_MAIN(){
    ...
    xTaskCreate(
            ...
            START_STK_SIZE,
            NULL,
            START_TASK_TEMP_PRIO,
            &START_TASK_HANDLER_TEMP
    );
