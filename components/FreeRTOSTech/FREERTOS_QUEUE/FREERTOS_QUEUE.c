...
#define vTASK3_QUEUE_FUNCTION_uxStackDepth      512
#define vTASK3_QUEUE_FUNCTION_uxPriority         2

#define vTASK4_QUEUE_FUNCTION_uxStackDepth      512
...
void vTASK4_QUEUE(void *pvParameters){
    ...
    while (1){
        ...
        //printf("vTASK4_QUEUE_NUM %d\r\n", vTASK4_QUEUE_NUM);
        printf("vTASK4_QUEUE %d\r\n", vTASK4_QUEUE_VALUE);
...
//    // 互斥锁相关
    xTaskCreate(
            ...
            (const configSTACK_DEPTH_TYPE)            512,// 任务堆栈大小 ，注意 ： 实际申请到的堆栈是 uxStackDepth 的 4 倍（ 一个 StackType_t 是 32 位  4 个字节）， 其中空闲任务 的堆栈大小为    configMINIMAL_STACK_SIZE。
            (void *  )                                "vTask5 param",// 传递给任务函数的参数
            (UBaseType_t)                                2,// 任务优先级 范围 0～ configMAX_PRIORITIES-1
