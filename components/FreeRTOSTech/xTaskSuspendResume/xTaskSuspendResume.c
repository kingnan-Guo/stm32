...
//int16_t num_SR = 0x00;
//int16_t num_SR2 = 0x00;
TaskHandle_t xHandleTask1_SR;
TaskHandle_t vTask2F_pxCreatedTask_SR; // 存放任务句柄
TaskHandle_t del_Handler_SR;
...
//    OLED_ShowString(2, 1, "vTask1F0");
    ...
    while (1) {
        if(num == 5){
            ...
            vTaskSuspend(vTask2F_pxCreatedTask_SR);// 挂起
        }
        if(num == 10){
            vTaskResume(vTask2F_pxCreatedTask_SR);// 重
        }
...
void SR_delFunction_SR(void *pvParameters){
    while (1) {
//        OLED_ShowNum(3, 1, num_SR, 5);
//        if(num_SR  ==  5){
//             vTaskDelete(del_Handler_SR); //删除开始任务
////            vTaskResume();//
//
//        }
    }
...
//    taskENTER_CRITICAL();           //进入临界区
    xTaskCreate(vTask1Function_SR, "Task1", 128, NULL, 3, &xHandleTask1_SR);
    xTaskCreate(
            ...
            (TaskHandle_t *)                        &vTask2F_pxCreatedTask_SR // 任务句柄，任务创建成功以后会返回次惹怒我的任务句柄， 这个 句柄其实就是任务的 任务堆栈，此参数 就用来保存这个任务句柄；其他API函数可能会使用到这个 句柄
    );

//    xTaskCreate(SR_delFunction_SR, "delFunction_SR", 128, NULL, 2, &del_Handler_SR);
