...
void vTASK1_EVENT_GROUP(void *pvParameters){
    ...
    while (1) {
        if(num == 5){
            printf(" delete task2\r\n");
            vTaskSuspend(TASK2_HANDLER_EVENT_GROUP);// 挂起
        }
        if(num == 10){
            vTaskResume(TASK2_HANDLER_EVENT_GROUP);// 重
        }
         printf(" vTASK1_EVENT_GROUP =%d\r\n",  num);
        //printf(" vTASK1_EVENT_GROUP\r\n");
...
void vTASK2_EVENT_GROUP(void *pvParameters){
    ...
    while (1) {
         printf(" vTASK2_EVENT_GROUP =%d\r\n",  num);
        //printf(" vTASK2_EVENT_GROUP\r\n");
...
void vDELETE_EVENT_GROUP(void *pvParameters){
    while (1) {
    }
}
//
...
void START_TASK_EVENT_GROUP(void *pvParameters)
{
    ...
    xTaskCreate(
            (TaskFunction_t)                           vTASK2_EVENT_GROUP, // 任务函数
            (char *  )                                    "vTask2",// 函数 名称， 任务名称长度不要超过  configMAX_TASK_NAME_LEN。
            (const configSTACK_DEPTH_TYPE)            vTASK2_EVENT_GROUP_FUNCTION_uxStackDepth,// 任务堆栈大小 ，注意 ： 实际申请到的堆栈是 uxStackDepth 的 4 倍（ 一个 StackType_t 是 32 位  4 个字节）， 其中空闲任务 的堆栈大小为    configMINIMAL_STACK_SIZE。
            // (uint32_t)                                         vTask2F_uxStackDepth,
            (void *  )                                "free2 0",// 传递给任务函数的参数
            (UBaseType_t)                                vTASK2_EVENT_GROUP_FUNCTION_uxPriority,// 任务优先级 范围 0～ configMAX_PRIORITIES-1
            (TaskHandle_t *)                         &TASK2_HANDLER_EVENT_GROUP // 任务句柄，任务创建成功以后会返回次惹怒我的任务句柄， 这个 句柄其实就是任务的 任务堆栈，此参数 就用来保存这个任务句柄；其他API函数可能会使用到这个 句柄
    );
