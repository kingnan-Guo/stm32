...
void delFunction(void *pvParameters){
//    OLED_ShowString(4, 1, pvParameters);
    while (1) {

        OLED_ShowNum(3, 1, num, 5);
        if(num  ==  5){
//            vTaskDelete(xHandleTask1); //删除开始任务
...
//    taskENTER_CRITICAL();           //进入临界区
    xTaskCreate(vTask1Function, "Task1", 128, NULL, 3, &xHandleTask1);
    xTaskCreate(
    ...
    xTaskCreate(delFunction, "delFunction", 128, NULL, 2, &del_Handler);
    vTaskDelete(StartTask_Handler); //删除开始任务;  为什么执行完成要删除？？？
