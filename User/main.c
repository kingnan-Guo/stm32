...
//        }
        ...
        if(Serial_package_RxFlag == 1){
            ...
            OLED_ShowString(4,1,"                ");// 擦除
            OLED_ShowString(4,1,Serial_package_RxPacket);
            // 如果相等 返回 0
            OLED_ShowString(3, 10, "   ");
            if(strcmp(Serial_package_RxPacket, "ON") == 0){
                OLED_ShowString(3, 10, "ON");
                //Serial_package_SendString("_ON");
                printf("xx_ON\r\n");
            }
            if(strcmp(Serial_package_RxPacket, "OFF") == 0){
                OLED_ShowString(3, 10, "OFF");
                Serial_package_SendString("_OFF");
            }
