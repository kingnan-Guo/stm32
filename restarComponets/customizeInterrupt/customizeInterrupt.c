...
void Interrupt_Init(){
    ...
    EXTI_InitStruct.EXTI_Trigger = EXTI_Trigger_Falling;// 下降沿 触发
    ...
    NVIC_InitStruct.NVIC_IRQChannel = EXTI15_10_IRQn;// 指定通道; 在stm32f10x.h文件中，找到 对应的通道，因为 要使用 pin 由于10-15 被合并成同一个通道，所以 选择 EXTI15_10_IRQn
    NVIC_InitStruct.NVIC_IRQChannelCmd = ENABLE;// 指定中断通道 使能
    NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = 1;// 抢占 优先级
