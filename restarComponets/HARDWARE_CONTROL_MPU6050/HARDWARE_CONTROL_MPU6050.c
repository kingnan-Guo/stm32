...
uint8_t HARDWARE_CONTROL_MPU6050_READ_REGISTER(uint8_t REGISTER_ADDRESS){
    ...
    I2C_SendData(I2C1, REGISTER_ADDRESS);
    // 发送后 需要等待 EV8 事件； 字节正在发送
    // while (I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTING) != SUCCESS);// 数据发送中： 这个也可以 替换下面一句
    while (I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTED) != SUCCESS);// 等待 数据发送完
    ...
    // 这里接收 一个字节
    I2C_AcknowledgeConfig(I2C1, DISABLE);// ACK = 0
...
void HARDWARE_CONTROL_MPU6050_INIT(){
    ...
    I2C_InitStruct.I2C_ClockSpeed = 100000;// 时钟频率 0~100KHZ 1000KHZ ~ 400KHZ
    ...
    // =====
    //配置电源管理寄存器 1
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_PWR_MGMT_1, 0x01);// 解除睡眠
    // 配置电源管理寄存器 1
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_PWR_MGMT_2, 0x00);// 选择陀螺仪 时钟 ， 六个轴 都不待机
    // 采样 预分频; 0x09 十分频
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_SMPLRT_DIV, 0x09);// 采样分配 为 10
    // 配置寄存器
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_CONFIG, 0x06);// 滤波参数给最大
    // 陀螺仪 配置
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_GYRO_CONFIG, 0x18);// 陀螺仪 最大量程
    // 加速度计配置寄存器
    HARDWARE_CONTROL_MPU6050_WRITE_REGISTER(MPU6050_ACCEL_CONFIG, 0x18);// 加速度计 最大量程
}
