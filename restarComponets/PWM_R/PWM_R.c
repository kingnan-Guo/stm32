...
void PWM_R_Init(void){
    ...
    TIM_TimeBaseInitStruct.TIM_Period = 100 -1 ;// 计数器 重装 器的值； 要在  0~ 65535 以内
    TIM_TimeBaseInitStruct.TIM_Prescaler = 720 - 1;// 预分频器 的值; 这里目的 是 定时 一秒钟；  要在 0~ 65535 以内
...
//    TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct
    ...
    TIM_OCInitStructure.TIM_OCNPolarity = ;// 输出比较极性
